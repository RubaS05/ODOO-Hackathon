...
import { Search, Clock, ArrowRight, CheckCircle } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
            {toCookOrders.length > 0 ? (toCookOrders.map((order) => renderOrderCard(order, () => handleStartPreparing(order.id), 'Start Preparing', <ArrowRight size={14}/>))) : (<div className="py-12 text-center text-muted-foreground text-xs border border-dashed border-border rounded-lg">
                No orders pending.
              </div>)}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
            {preparingOrders.length > 0 ? (preparingOrders.map((order) => renderOrderCard(order, () => handleCompletePreparing(order.id), 'Mark Completed', <CheckCircle size={14}/>))) : (<div className="py-12 text-center text-muted-foreground text-xs border border-dashed border-border rounded-lg">
                No active preps.
              </div>)}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
            {completedOrders.length > 0 ? (completedOrders.map((order) => renderOrderCard(order, () => { }, 'Completed Check', <CheckCircle size={14} className="text-emerald-500"/>))) : (<div className="py-12 text-center text-muted-foreground text-xs border border-dashed border-border rounded-lg">
