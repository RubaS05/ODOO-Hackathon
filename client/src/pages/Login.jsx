    ...
    return (<div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md glass border border-border/40 shadow-2xl relative z-10">
        ...
        <CardContent className="px-8 pb-8">
          {errorMsg && (<div className="mb-5 p-3 rounded-lg bg-destructive/15 border border-destructive/20 text-xs text-destructive font-medium animate-shake">
              {errorMsg}
            </div>)}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="relative">
              ...
              <Input label="Username or Email" id="emailOrUsername" placeholder="Enter admin, alice, or chef" className="pl-9" error={errors.emailOrUsername?.message} {...register('emailOrUsername')}/>
            </div>

            <div className="relative">
              <Lock size={16} className="absolute left-3 top-9 text-muted-foreground z-10"/>
              <Input label="Password" id="password" type="password" placeholder="Enter password (any character)" className="pl-9" error={errors.password?.message} {...register('password')}/>
            </div>

            <div className="flex items-center justify-between mt-2">
              <label className="flex items-center gap-2 text-xs font-semibold text-muted-foreground cursor-pointer select-none">
                <input type="checkbox" className="rounded border-input text-primary focus:ring-primary h-4 w-4 bg-background" {...register('rememberMe')}/>
                Remember Me
              </label>
