...
export const Orders = () => {
    ...
    return (<div className="space-y-6">
      ...
      <Card className="glass border border-border/40">
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex bg-card p-1 rounded-lg border border-border/60 w-full md:w-auto">
            {['all', 'draft', 'pending', 'preparing', 'completed'].map((status) => (<button key={status} onClick={() => setStatusFilter(status)} className={`flex-1 md:flex-initial px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all cursor-pointer ${statusFilter === status
          ...
          {filteredOrders.length > 0 ? (filteredOrders.map((order) => (<TableRow key={order.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1.5">
                    ...
                    {order.status === 'draft' && (<>
                        <Button variant="secondary" size="icon" className="h-8 w-8 cursor-pointer text-primary" onClick={() => handleEditDraft(order)}>
                          <Edit2 size={14}/>
                        </Button>
                        <Button variant="ghost" size="icon" className="h-8 w-8 cursor-pointer text-destructive hover:bg-destructive/10" onClick={() => handleDeleteOrder(order.id)}>
                          <Trash2 size={14}/>
                        </Button>
                      </>)}
                  </div>
      ...
      <Modal isOpen={!!selectedOrder} onClose={() => setSelectedOrder(null)} title={`Order Details: ${selectedOrder?.orderNumber}`} size="md">
        {selectedOrder && (<div className="space-y-5 text-xs font-mono">
            {/* Details header */}
            <div className="space-y-1 bg-accent/20 p-3 rounded-lg border border-border/20">
              ...
              {selectedOrder.tableNumber && (<div className="flex justify-between text-xxs">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {selectedOrder.items.map((item) => (<div key={item.id} className="flex justify-between font-mono text-xxs">
                    <div>
            ...
            <div className="border-t border-border pt-3 space-y-1">
              ...
              {selectedOrder.discount > 0 && (<div className="flex justify-between text-emerald-500 font-bold">
                  <span>Discount</span>
            ...
            {/* Payment Details */}
            {selectedOrder.paymentMethod && (<div className="border-t border-border/40 pt-2 space-y-0.5 text-xxs">
                ...
                {selectedOrder.paymentDetails?.reference && (<p className="truncate">Ref: {selectedOrder.paymentDetails.reference}</p>)}
              </div>)}

            {/* Notes */}
            {selectedOrder.notes && (<div className="p-2.5 rounded bg-amber-500/10 border border-amber-500/20 text-xxs text-amber-500 italic">
            ...
            <div className="flex gap-2 border-t border-border/40 pt-4 font-sans">
              ...
              {selectedOrder.status === 'draft' ? (<Button className="flex-1 font-bold text-xs h-9 cursor-pointer" onClick={() => handleEditDraft(selectedOrder)}>
                  Edit Order
                </Button>) : (<Button variant="ghost" className="flex-1 text-xs h-9 cursor-pointer" onClick={() => setSelectedOrder(null)}>
                  Close View
                </Button>)}
            </div>
          </div>)}
