...
import { Search, ShoppingCart, UserPlus, Receipt, ChefHat, Ticket, Check, QrCode, CreditCard, DollarSign } from 'lucide-react';
...
export const POSOrder = () => {
    // Store selectors
    const { products, categories, cart, activeCustomer, activeTable, activeCoupon, orderType, cartNotes, customers, coupons, paymentMethods, addToCart, removeFromCart, updateCartQuantity, clearCart, assignCustomer, assignTable, applyCoupon, setOrderType, setCartNotes, getCartTotals, createOrder, addCustomer, } = usePOSStore();
    // Search & Filter State
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex bg-card p-1 rounded-lg border border-border/60">
          {['dine-in', 'takeaway', 'delivery'].map((type) => (<button key={type} onClick={() => setOrderType(type)} className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all cursor-pointer ${orderType === type
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <section className="lg:col-span-5 space-y-4">
          <Card className="glass border border-border/40">
            <div className="p-4 space-y-4">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 max-w-full">
                <button onClick={() => setSelectedCategoryId(null)} className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-all cursor-pointer ${selectedCategoryId === null
            ? 'bg-primary text-primary-foreground border-primary'
            : 'bg-accent/40 text-muted-foreground border-border/40 hover:text-foreground hover:bg-accent'}`}>
                  All Items
                </button>
                {categories.map((cat) => (<button key={cat.id} onClick={() => setSelectedCategoryId(cat.id)} style={{ borderColor: selectedCategoryId === cat.id ? cat.color : 'transparent' }} className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-all cursor-pointer ${selectedCategoryId === cat.id
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-3 overflow-y-auto max-h-[500px] pr-1">
            ...
            return (<Card key={prod.id} className="cursor-pointer hover:border-primary/50 transition-all select-none hover:scale-[1.02] flex flex-col justify-between" onClick={() => handleAddProduct(prod)}>
        ...
        <section className="lg:col-span-4 space-y-4">
          <Card className="glass border border-border/40 flex flex-col h-[650px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length > 0 ? (cart.map((item) => (<div key={item.product.id} className="flex items-center justify-between gap-2 p-2.5 rounded-lg bg-accent/20 border border-border/20">
                    ...
                    <div className="flex items-center border border-border rounded-md bg-background">
                      <button onClick={() => updateCartQuantity(item.product.id, item.quantity - 1)} className="px-2 py-1 text-xs hover:bg-accent font-bold cursor-pointer">
                        -
                      </button>
                  ...
                  </div>))) : (<div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground p-4">
            ...
            {/* Cart Meta / Notes */}
            {cart.length > 0 && (<div className="px-4 py-2 border-t border-border/30 bg-card/10">
            ...
            <div className="p-4 border-t border-border bg-card/40 space-y-1.5 text-xs">
              ...
              {discount > 0 && (<div className="flex justify-between text-emerald-500 font-semibold">
                  <span>Discount</span>
                  <span className="font-mono">-${discount.toFixed(2)}</span>
                </div>)}
              {activeTable && (<div className="flex justify-between text-primary font-semibold">
        ...
        <section className="lg:col-span-3 space-y-4">
          <Card className="glass border border-border/40 p-4 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-1 bg-accent/40 p-1 rounded-lg">
              {['cash', 'card', 'upi'].map((method) => (<button key={method} onClick={() => setSelectedPaymentType(method)} className={`py-2 rounded-md text-xxs font-bold uppercase transition-all cursor-pointer flex flex-col items-center gap-1 ${selectedPaymentType === method
                ? 'bg-primary text-primary-foreground shadow-sm'
                : 'text-muted-foreground hover:text-foreground'}`}>
            ...
            <div className="space-y-4 min-h-[220px]">
              {selectedPaymentType === 'cash' && (<div className="space-y-3 animate-in fade-in duration-200">
                  <Input label="Amount Received ($)" placeholder="0.00" type="number" step="0.01" value={cashAmountReceived} onChange={(e) => setCashAmountReceived(e.target.value)}/>

                  {/* Cash received shortcuts */}
                  <div className="grid grid-cols-3 gap-1">
                    {[10, 20, 50, 100].map((amt) => (<button key={amt} onClick={() => handleQuickCash(amt)} className="py-1 px-2 text-xxs font-bold bg-accent/50 rounded hover:bg-accent border border-border/20 cursor-pointer">
                        ${amt}
                      </button>))}
                    <button onClick={() => handleQuickCash(total)} className="col-span-2 py-1 px-2 text-xxs font-black bg-primary/10 text-primary border border-primary/20 rounded hover:bg-primary/20 cursor-pointer">
              ...
              {selectedPaymentType === 'card' && (<div className="space-y-3 animate-in fade-in duration-200">
                  <p className="text-xxs text-muted-foreground">
                    Process the payment on your external card terminal, then enter the authorization/reference reference below.
                  </p>
                  <Input label="Transaction Reference" placeholder="TXN-XXXXXX" value={cardReference} onChange={(e) => setCardReference(e.target.value)}/>
                </div>)}

              {selectedPaymentType === 'upi' && (<div className="space-y-3 text-center animate-in fade-in duration-200">
            ...
            <Button className="w-full font-black py-5 cursor-pointer text-sm" onClick={handleCheckoutPayment} disabled={cart.length === 0}>
              Complete Checkout
            </Button>
          </Card>
        </section>
      ...
      <Modal isOpen={customerModalOpen} onClose={() => setCustomerModalOpen(false)} title="Assign Customer to Order">
        <div className="space-y-5">
        ...
        }} className={`flex items-center justify-between p-2.5 rounded-lg border text-xs cursor-pointer transition-colors ${activeCustomer === null ? 'border-primary bg-primary/10' : 'border-border/60 hover:bg-accent/40'}`}>
                <span className="font-semibold">Walk-in Guest</span>
                {activeCustomer === null && <Check size={14} className="text-primary"/>}
              </div>
              {customers.map((cust) => (<div key={cust.id} onClick={() => {
                assignCustomer(cust);
                setCustomerModalOpen(false);
            }} className={`flex items-center justify-between p-2.5 rounded-lg border text-xs cursor-pointer transition-colors ${activeCustomer?.id === cust.id ? 'border-primary bg-primary/10' : 'border-border/60 hover:bg-accent/40'}`}>
                  <div>
          ...
          <form onSubmit={handleAddNewCustomer} className="space-y-3">
            ...
            <Input placeholder="Full Name" value={newCustomerName} onChange={(e) => setNewCustomerName(e.target.value)} className="h-9 text-xs"/>
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="Email" type="email" value={newCustomerEmail} onChange={(e) => setNewCustomerEmail(e.target.value)} className="h-9 text-xs"/>
              <Input placeholder="Phone Number" value={newCustomerPhone} onChange={(e) => setNewCustomerPhone(e.target.value)} className="h-9 text-xs"/>
            </div>
      ...
      <Modal isOpen={couponModalOpen} onClose={() => setCouponModalOpen(false)} title="Apply Discount Coupon">
        <div className="space-y-4">
          ...
          <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
        ...
        }} className={`p-3 rounded-lg border text-xs cursor-pointer flex justify-between items-center transition-colors ${activeCoupon === null ? 'border-primary bg-primary/10' : 'border-border/60 hover:bg-accent/40'}`}>
            ...
            return (<div key={coup.id} onClick={() => {
                    if (!eligible)
                        return;
                    applyCoupon(coup);
                    setCouponModalOpen(false);
                }} className={`p-3 rounded-lg border text-xs flex justify-between items-center transition-colors ${!eligible ? 'opacity-40 cursor-not-allowed border-border/20' : 'cursor-pointer'} ${activeCoupon?.id === coup.id ? 'border-primary bg-primary/10' : 'border-border/60 hover:bg-accent/40'}`}>
                  <div>
                    <div className="flex items-center gap-2">
                    ...
                    {coup.minAmount && (<p className="text-xxs text-muted-foreground mt-1">
      ...
      <Modal isOpen={receiptModalOpen} onClose={() => {
            setReceiptModalOpen(false);
            setCheckoutSuccess(null);
        }} title="POS Order Submitted" size="sm">
        {checkoutSuccess && (<div className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="p-4 border-2 border-dashed border-border rounded-lg bg-card text-xs font-mono space-y-3">
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {checkoutSuccess.items.map((oi) => (<div key={oi.id} className="flex justify-between text-xxs">
              ...
              <div className="border-t border-border/60 pt-2 space-y-1 text-xxs">
                ...
                {checkoutSuccess.discount > 0 && (<div className="flex justify-between text-emerald-500 font-bold">
                    <span>Discount</span>
              ...
              {checkoutSuccess.paymentDetails && (<div className="border-t border-border/40 pt-2 text-xxs space-y-0.5">
                  ...
                  {checkoutSuccess.paymentMethod === 'card' && (<p className="truncate">Ref: {checkoutSuccess.paymentDetails.reference}</p>)}
                </div>)}
            </div>

            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 font-bold text-xs h-9" onClick={() => {
                window.print();
            }}>
                Print Receipt
              </Button>
              <Button className="flex-1 font-bold text-xs h-9" onClick={() => {
                setReceiptModalOpen(false);
                setCheckoutSuccess(null);
            }}>
                New Transaction
