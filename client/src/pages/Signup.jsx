...
export const Signup = () => {
    ...
    const onSubmit = async (data) => {
        ...
        setErrorMsg(null);
        try {
            await apiService.auth.signup(data.fullName, data.email, data.password);
            setSuccess(true);
            reset();
        }
        catch (err) {
    ...
    return (<div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md glass border border-border/40 shadow-2xl relative z-10">
        ...
        <CardContent className="px-8 pb-8">
          {errorMsg && (<div className="mb-5 p-3 rounded-lg bg-destructive/15 border border-destructive/20 text-xs text-destructive font-medium">
              {errorMsg}
            </div>)}

          {success ? (<div className="text-center space-y-4 py-4 animate-in fade-in zoom-in duration-300">
              ...
              <div className="relative">
                ...
                <Input label="Full Name" id="fullName" placeholder="e.g. John Doe" className="pl-9" error={errors.fullName?.message} {...register('fullName')}/>
              </div>

              <div className="relative">
                <Mail size={16} className="absolute left-3 top-9 text-muted-foreground z-10"/>
                <Input label="Email Address" id="email" type="email" placeholder="e.g. john@pos.com" className="pl-9" error={errors.email?.message} {...register('email')}/>
              </div>
              ...
              <div className="relative">
                ...
                <Input label="Password" id="password" type="password" placeholder="At least 6 characters" className="pl-9" error={errors.password?.message} {...register('password')}/>
              </div>
