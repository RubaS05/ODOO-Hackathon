...
export const TableManagement = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-card p-1.5 rounded-lg border border-border/60 max-w-sm">
        {floors.map((floor) => (<button key={floor} onClick={() => setSelectedFloor(floor)} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all cursor-pointer ${selectedFloor === floor
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {filteredTables.map((table) => (<Card key={table.id} onClick={() => handleTableClick(table)} className={`cursor-pointer transition-all hover:scale-[1.03] duration-200 select-none ${getStatusColor(table.status)}`}>
      ...
      <Modal isOpen={createModalOpen} onClose={() => setCreateModalOpen(false)} title="Add New Table">
        <form onSubmit={handleCreateTable} className="space-y-4">
          <Input label="Table Number" placeholder="e.g. 105 or A3" value={newTableNo} onChange={(e) => setNewTableNo(e.target.value)} required/>
          <Input label="Seats Count" type="number" min="1" max="20" value={newTableSeats} onChange={(e) => setNewTableSeats(Number(e.target.value))} required/>
          <Select label="Floor Location" value={newTableFloor} onChange={(e) => setNewTableFloor(e.target.value)}>
      ...
      <Modal isOpen={!!activeTableDetail} onClose={() => setActiveTableDetail(null)} title={`Table ${activeTableDetail?.table?.number} Details`} size="md">
        {activeTableDetail && (<div className="space-y-5 text-sm">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-1.5">
                <Button variant="outline" size="sm" onClick={() => handleUpdateStatus(activeTableDetail.table.id, 'available')} className={`text-xxs h-8 ${activeTableDetail.table.status === 'available' ? 'bg-emerald-500/15 text-emerald-500 hover:bg-emerald-500/25 border-emerald-500/30' : ''}`}>
                  Available
                </Button>
                <Button variant="outline" size="sm" onClick={() => handleUpdateStatus(activeTableDetail.table.id, 'reserved')} className={`text-xxs h-8 ${activeTableDetail.table.status === 'reserved' ? 'bg-amber-500/15 text-amber-500 hover:bg-amber-500/25 border-amber-500/30' : ''}`}>
                  Reserve
                </Button>
              </div>
            </div>
            ...
            {activeTableDetail.order ? (<div className="space-y-4">
                <div className="flex justify-between items-center">
                ...
                <div className="p-3 bg-accent/20 rounded-lg border border-border/20 space-y-2">
                  ...
                  <div className="border-t border-border/20 pt-2 space-y-1">
                    {activeTableDetail.order.items.map((oi) => (<div key={oi.id} className="flex justify-between text-xxs font-mono">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" className="flex-1 text-xs h-9 cursor-pointer" onClick={() => {
                  ...
                  <Button className="flex-1 text-xs h-9 cursor-pointer font-bold" onClick={() => {
                    assignTable(activeTableDetail.table);
                    usePOSStore.getState().loadDraftToCart(activeTableDetail.order);
                    navigate('/');
                }}>
              ...
              </div>) : (<div className="py-4 text-center text-muted-foreground text-xs">
                No active order found. Table status seems occupied but has no orders.
                <Button className="mt-3 text-xs w-full cursor-pointer" onClick={() => {
                    assignTable(activeTableDetail.table);
                    navigate('/');
                }}>
                  Create Order
            ...
            <div className="border-t border-border/40 pt-4 flex justify-end">
              <Button variant="ghost" className="text-xs text-destructive hover:bg-destructive/10 cursor-pointer h-9" onClick={() => handleDeleteTableClick(activeTableDetail.table.id)}>
                Delete Table
              </Button>
            </div>
          </div>)}
