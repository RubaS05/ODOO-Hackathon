...
import { UserPlus, Shield, Mail, Key, Edit2, Archive, Trash2 } from 'lucide-react';
...
export const UserManagement: React.FC = () => {
  ...
  const [role, setRole] = useState<'admin' | 'manager' | 'cashier' | 'chef'>('cashier');
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case 'chef':
        return <Badge variant="warning">Kitchen Chef</Badge>;
      case 'cashier':
  ...
  return (
    <div className="space-y-6">
              ...
              <TableCell className="text-right">
                <div className="flex justify-end gap-1.5">
                  ...
                  <Button
                    variant="outline"
                    size="icon"
                    className={`h-8 w-8 cursor-pointer ${user.status === 'active' ? 'text-muted-foreground' : 'text-emerald-500'}`}
      ...
      <Modal
        ...
        title={selectedUser ? 'Edit Staff Profile' : 'Register New Employee'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
            label="Full Name"
            placeholder="e.g. Robert Chef"
            value={name}
          ...
          <Input
            label="Email Address"
            type="email"
            placeholder="e.g. robert@pos.com"
            value={email}
